const STORAGE_KEY='quizTopScores'; let playerName=''; let score=0; let wrongStreak=0; let selectedQuestions=[]; let currentIndex=0; let answersLog=[];
const startScreen=document.getElementById('start-screen'); const playerInput=document.getElementById('player-name'); const startBtn=document.getElementById('start-btn'); const quizContainer=document.getElementById('quiz-container'); const playerLabel=document.getElementById('player-label'); const scoreLabel=document.getElementById('score'); const progressLabel=document.getElementById('question-index'); const questionText=document.getElementById('question-text'); const answersDiv=document.getElementById('answers'); const feedbackDiv=document.getElementById('feedback'); const nextBtn=document.getElementById('next-btn'); const summarySection=document.getElementById('summary'); const summaryPlayer=document.getElementById('summary-player'); const summaryScore=document.getElementById('summary-score'); const listCorrect=document.getElementById('list-correct'); const listWrong=document.getElementById('list-wrong'); const playAgainBtn=document.getElementById('play-again-btn');
function randInt(max){return Math.floor(Math.random()*max);} function pickProgressiveQuestions(pool){const result=[]; for(let level=1;level<=10;level++){const candidates=pool.filter(q=>q.difficulty===level); const chosen=candidates[randInt(candidates.length)]; result.push(chosen);} return result;}
function resetGame(){score=0; wrongStreak=0; currentIndex=0; answersLog=[]; selectedQuestions=pickProgressiveQuestions(QUESTIONS_POOL); scoreLabel.textContent=score; feedbackDiv.textContent=''; nextBtn.disabled=true; progressLabel.textContent='1';}
function startGame(){playerName=(playerInput.value||'').trim(); if(!playerName){alert('Per favore inserisci il tuo nome.'); return;} startScreen.classList.add('hidden'); quizContainer.classList.remove('hidden'); playerLabel.textContent=`Giocatore: ${playerName}`; resetGame(); renderQuestion();}
function renderQuestion(){const q=selectedQuestions[currentIndex]; questionText.textContent=q.question; answersDiv.innerHTML=''; feedbackDiv.textContent=''; nextBtn.disabled=true; q.options.forEach((opt,idx)=>{const btn=document.createElement('button'); btn.className='answer-btn'; btn.textContent=opt; btn.addEventListener('click',()=>onAnswer(idx)); answersDiv.appendChild(btn);});}
